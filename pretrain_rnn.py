...
class RNNSeq2Seq(nn.Module):
    def __init__(self, len_vocab, embed_size, hidden_size, n_layers):
        super(RNNSeq2Seq, self).__init__()
        self.encoder = Encoder(len_vocab, embed_size, hidden_size, n_layers=n_layers)
        self.decoder = Decoder(embed_size, hidden_size, len_vocab, n_layers=n_layers)

    def forward(self, src, trg, teacher_forcing_ratio=0.5): # (T,B)
        batch_size = src.size(dim=1)
...
def parse_arguments():
    ...
    parser.add_argument('--train_data', type=str, default='../data/train_24.csv', help='train corpus (.csv)')
    parser.add_argument('--test_data', type=str, default='../data/test_24.csv', help='test corpus (.csv)')
...
def evaluate(model, val_loader, vocab):
    ...
    for b, data in enumerate(val_loader):
        sm1, sm2 = torch.t(data.cuda()), torch.t(data.cuda()) # (T,B)
        with torch.no_grad():
...
def main():
    ...
    model = RNNSeq2Seq(len(vocab), embed_size, hidden_size, args.n_layer).cuda()
    optimizer = optim.Adam(model.parameters(), lr=args.lr)
    train_dataset = Seq2seqDataset(pd.read_csv(args.train_data)['canonical_smiles'].values, vocab)
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.n_worker)
    val_dataset = Seq2seqDataset(pd.read_csv(args.test_data)['canonical_smiles'].values, vocab)
